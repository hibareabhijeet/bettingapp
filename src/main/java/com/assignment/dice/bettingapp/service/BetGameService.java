...
public class BetGameService {
    ...
    private UserEntity validateBet(BetGame betGame) {
        if(betGame.getAmount() < 1 || (betGame.getAmount() < amountMinLimit || betGame.getAmount() > amountMaxLimit)){
            throw new SystemException(String.format("Bet amount must be in Betting range [%s : %s]",amountMinLimit,amountMaxLimit));
        }
